...
func getPokemon(name string) (pokemon, error) {
	body, err := getFromCacheOrServer(fmt.Sprintf("https://pokeapi.co/api/v2/pokemon/%s", name))
	...
	var types []string
	for _, value := range response.Types {
		types = append(types, value.Type.Name)
	}

	stats := make(map[string]int)
	for _, value := range response.Stats {
		stats[value.Stat.Name] = value.BaseStat
	}
